...
const MainComponent: React.FC = () => {
  ...
  const handleDeleteBoard = (e: React.MouseEvent, boardId: string) => {
    e.stopPropagation(); // Предотвращаем клик по самой доске
    dispatch(removeBoard(boardId)); // Удаляем без подтверждения
  };
    ...
    <div className="main-content">
      ...
      {boards.length > 0 && (
        <div className="boards-container">
          {/* <h3 className="boards-title">Мои доски ({boards.length})</h3> */}
